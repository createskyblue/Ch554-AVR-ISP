...
unsigned char ISP_Trans_Package(unsigned char a, unsigned char b, unsigned char c, unsigned char d)
{
    ...
    SPI_MasterData(b);
    SPI_MasterData(c);
    return SPI_MasterData(d);
}

//发送空回复/INSYNC+OK/NOSYNC
void ISP_Send_Empty_Reply()
{
    if (Sync_CRC_EOP == CDC_UART_read())
    {
        CDC_UART_write(Resp_STK_INSYNC);
        CDC_UART_write(Resp_STK_OK);
    }
    else
    {
        ISP_Error_Count++;
        CDC_UART_write(Resp_STK_NOSYNC);
    }
}
...
void ISP_Fill_Buffer(unsigned char n)
{
    ...
    for (x = 0; x < n; x++)
    {
        ISP_Data_Buffer[x] = CDC_UART_read();
    }
}
...
void ISP_Start_Programming_Mode()
{
    ...
    SPI_SetMasterMode0();
	SPI_SetMasterIO();
	SPI_SetClockDivider(192);

    // See AVR datasheets, chapter "SERIAL_PRG Programming Algorithm":

    // Pulse RESET after PIN_SCK is low:
    SCK = 0;
    mDelaymS(20); // discharge PIN_SCK, value arbitrarily chosen
    ISP_Reset_Target(0);
...
void ISP_Write_Flash(unsigned int length)
{
    ...
    {
        CDC_UART_write(Resp_STK_INSYNC);
        CDC_UART_write(ISP_Write_Flash_Page(length));
    }
    else
    {
        ISP_Error_Count++;
...
void ISP_Program_Page()
{
    ...
    CDC_UART_write(Resp_STK_FAILED);
...
void ISP_Process_Data()
{
    ...
    case Cmnd_STK_GET_SIGN_ON:
        //'1'
        if (CDC_UART_read() == Sync_CRC_EOP)
        {
            CDC_UART_write(Resp_STK_INSYNC);
            CDC_UART_print(STK_SIGN_ON_MESSAGE);
            CDC_UART_write(Resp_STK_OK);
        }
        else
        {
    ...
    case Cmnd_STK_GET_PARAMETER:
        //'A'
        ISP_Get_Device_Parameter(CDC_UART_read());
        break;
    ...
    case Cmnd_STK_LOAD_ADDRESS:
        //'U'
        ISP_Operating_Addr = CDC_UART_read();
        ISP_Operating_Addr += 256 * CDC_UART_read();
        ISP_Send_Empty_Reply();
        break;

    case Cmnd_STK_PROG_FLASH:
        //0x60
        CDC_UART_read(); // low addr
        CDC_UART_read(); // high addr
        ISP_Send_Empty_Reply();
        break;
    ...
    default:
        ISP_Error_Count++;
        if (Sync_CRC_EOP == CDC_UART_read())
            CDC_UART_write(Resp_STK_UNKNOWN);
        else
            CDC_UART_write(Resp_STK_NOSYNC);
    }
}
